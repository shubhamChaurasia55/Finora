...
router.post("/add", protact, async (req, res) => {
  try {
    ...
    const newTransaction = await pool.query(
      `INSERT INTO transactions (amount, description, category, type, transaction_date, user_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
      [amount, description, category, type, transaction_date, req.user.id],
    );
...
router.get("/all", protact, async (req, res) => {
  try {
    ...
    if (search) {
      ...
      query += `
                AND description ILIKE $${values.length}
            `;
    }
...
router.get("/summary", protact, async (req, res) => {
  try {
    const summary = await pool.query(
      "SELECT SUM(amount) as total, type FROM transactions WHERE user_id = $1 GROUP BY type",
      [req.user.id],
    );

    const totalIncome =
      summary.rows.find((item) => item.type === "income")?.total || 0;
    const totalExpense =
      summary.rows.find((item) => item.type === "expense")?.total || 0;
...
router.patch("/update/:id", protact, async (req, res) => {
  try {
    ...
    const updatedTransaction = await pool.query(
      `UPDATE transactions SET amount = $1, description = $2, category = $3, type = $4, transaction_date = $5 WHERE id = $6 AND user_id = $7 RETURNING *`,
      [amount, description, category, type, transaction_date, id, req.user.id],
    );
